...
function emit(w, counter, circular, o, n, p) {
    if (test.isString(o)) {
        ...
        for (var idx in o) {
            emit(child, circular, counter, o[idx], '_' + idx);
        }
        ...
        for (var idx in o) {
            ...
            if (existing !== undefined) {
                ...
                emit(e, counter, circular, o[idx], idx);
            }
        }
        e.endElement(elem);
    }
...
function toXml(object, options) {
    ...
    w.startDocument()
    emit(w, 1, circular, object, options.rootName, options.defaultPrefix);
    w.endDocument();
